...
class Trainer:
    def __init__(self, config):
        self.config = config
        self.dataset = (
            datasets_map[self.config.train.dataset](config.dataset)
            .pyg_dataset()
    ...
    def visualize(self, dataset: EllipticDataset, time_step: int, save_to=None):
        pred_scores, pred_labels = self.test(
            dataset.pyg_dataset().to(self.config.train.device), labeled_only=True
        )
        ...
        nx.draw_networkx(
            G, nodelist=node_list, node_color=node_color, node_size=6, with_labels=False
        )
        if save_to:
