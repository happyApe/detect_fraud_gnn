...
def normalize(feature_matrix):
    ...
    stdev = th.sqrt(
        th.sum((feature_matrix - mean) ** 2, axis=0) / feature_matrix.shape[0]
    )
...
def get_model_class_predictions(model, g, features, labels, device, threshold=None):
    ...
    if not threshold:
        return (
            unnormalized_preds.argmax(axis=1).detach().numpy(),
            pred_proba[:, 1].detach().numpy(),
        )
    return (
        np.where(pred_proba.detach().numpy() > threshold, 1, 0),
        pred_proba[:, 1].detach().numpy(),
    )
...
def save_model(g, model, model_dir, id_to_node, mean, stdev):
    ...
    for ntype, mapping in id_to_node.items():
        ...
        cols = {
            "val" + str(i + 1) + ":Double": node_feats[:, i] for i in range(num_feats)
        }
...
if __name__ == "__main__":
    ...
    print(
        "numpy version:{} PyTorch version:{} DGL version:{}".format(
            np.__version__, th.__version__, dgl.__version__
        )
    )
